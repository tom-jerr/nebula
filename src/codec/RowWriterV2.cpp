...
RowWriterV2::RowWriterV2(const meta::NebulaSchemaProvider* schema, bool isVectorColumns)
...
RowWriterV2::RowWriterV2(const meta::NebulaSchemaProvider* schema,
    ...
    : isVectorColumns_(isVectorColumns), schema_(schema), finished_(false), outOfSpaceStr_(false) {
...
RowWriterV2::RowWriterV2(RowReaderWrapper& reader, bool isVectorColumn, size_t index)
    : RowWriterV2(reader.getSchema(), isVectorColumn) {
  if (!isVectorColumns_) {
    ...
    Value v = reader.getVectorValueByIndex(index);
    switch (v.type()) {
      case Value::Type::NULLVALUE:
        setNullVec(index);
        break;
      case Value::Type::VECTOR:
        setVec(index, v.moveVector());
        break;
      default:
        LOG(FATAL) << "Invalid data: " << v << ", type: " << v.typeName();
        isSet_[0] = false;
